...
class RewardDistTrainer(SupervisedTrainer):
    ...
    def eval(self) -> dict[str, Any]:
        ...
        for batch in eval_dataloader:
            ...
            num_correct_predictions += (higher_end_rewards[:,0] > lower_end_rewards[:,0]).sum()
        ...
        if is_main_process():
            ...
            h_rewards = [f'({reward[0]:.6f}, {reward[1]:.6f})' for reward in higher_end_rewards.tolist()]
            l_rewards = [f'({reward[0]:.6f}, {reward[1]:.6f})' for reward in lower_end_rewards.tolist()]
    ...
    def loss(
        ...
        # higher_end_reward, lower_end_reward are  (B, 2)
        dist = torch.distributions.Normal(0. , 1.)

        # TOKEN-WISE NOT IMPLEMENTED YET
        if self.args.loss_type == 'token-wise':
            raise NotImplementedError("Token wise loss not yet.") 
            losses = []
            ...
            mean_high, log_var_high = higher_end_reward[:,0], higher_end_reward[:,1]
            mean_low, log_var_low = lower_end_reward[:,0], lower_end_reward[:,1]
            dist_inp_args = (mean_high-mean_low)/torch.sqrt(log_var_high.exp() + log_var_low.exp())
            # loss = -F.logsigmoid(higher_end_reward - lower_end_reward).mean()
            # with torch.no_grad():
                # pdf_val = dist.log_prob(dist_inp_args).exp().to(self.args.device)
                # cdf_val = dist.cdf(dist_inp_args).to(self.args.device)
            # loss = ((-pdf_val/(cdf_val+1e-8))*dist_inp_args).mean()
            loss = -torch.log(dist.cdf(dist_inp_args) + 1e-8).mean()
        ...
        accuracy = (higher_end_reward[:,0] > lower_end_reward[:,0]).float().mean()  # size = ()
        return {
            'loss': loss,  # size = ()
            'higher_end_reward': higher_end_reward[:,0].contiguous(),  # size = (B,)
            'lower_end_reward': lower_end_reward[:,0].contiguous(),  # size = (B,)
            'higher_rewards': higher_rewards[:,:,0].contiguous(),  # size = (B, L)
            'lower_rewards': lower_rewards[:,:,0].contiguous(),  # size = (B, L)
            'accuracy': accuracy,  # size = ()
            'higher_end_reward_std': higher_end_reward[:,1].contiguous().exp(),  # size = (B,)
            'lower_end_reward_std': lower_end_reward[:,1].contiguous().exp(),  # size = (B,)
            'higher_rewards_std': higher_rewards[:,:,1].contiguous().exp(),  # size = (B, L)
            'lower_rewards_std': lower_rewards[:,:,1].contiguous().exp(),  # size = (B, L)
        }
    ...
    def train_step(
        ...
        accuracy = get_all_reduce_mean(accuracy)
        avg_mean_r_high_end = get_all_reduce_mean(loss_dict['higher_end_reward'])
        avg_mean_r_low_end = get_all_reduce_mean(loss_dict['lower_end_reward'])
        avg_std_r_high_end = get_all_reduce_mean(loss_dict['higher_end_reward_std'])
        avg_std_r_low_end = get_all_reduce_mean(loss_dict['lower_end_reward_std'])

        return {
            'train/loss': loss.item(),
            'train/accuracy': accuracy.item(),
            'train/lr': self.model.optimizer.param_groups[0]['lr'],
            'train/avg_mean_reward_higher_end': avg_mean_r_high_end,
            'train/avg_mean_reward_lower_end': avg_mean_r_low_end,
            'train/avg_std_reward_higher_end': avg_std_r_high_end,
            'train/avg_std_reward_lower_end': avg_std_r_low_end,
        }
