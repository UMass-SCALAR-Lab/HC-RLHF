...
class SimPOTrainer(SupervisedTrainer):
    TRAINING_TYPE = 'simpo'
    ...
    def loss(  # pylint: disable=too-many-locals
        ...
        **kwargs
        ...
        for i in range(batch_size):
            ...
            len_better_seq_slice = better_end_index + 1 - diverge_index
            len_worse_seq_slice = worse_end_index + 1 - diverge_index
            ...
            if(margin is None):
                losses.append(-F.logsigmoid(self.scale_coeff * ((better_log_prob/len_better_seq_slice) - (worse_log_prob/len_worse_seq_slice))-self.args.margin))
            else:
                losses.append(-F.logsigmoid(self.scale_coeff * ((better_log_prob/len_better_seq_slice) - (worse_log_prob/len_worse_seq_slice))-margin))
            better_sample_rewards.append(self.scale_coeff * (better_log_prob/len_better_seq_slice).detach())
            worse_sample_rewards.append(self.scale_coeff * (worse_log_prob/len_worse_seq_slice).detach())
