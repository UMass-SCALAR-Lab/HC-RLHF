...
from safe_rlhf.algorithms.reinforce_lag.trainer import ReinforceCILagTrainer
...
def parse_arguments() -> argparse.Namespace:
    ...
    # We add the Seldonian args here
    training_parser.add_argument(
        '--fail_prob',
        type=float,
        default=0.1,
        help='The failure probability for the Seldonian algorithm',
    )
    training_parser.add_argument(
        '--safety_dataset_size',
        type=int,
        default=1000,
        help='The Number of datapoints in the safety dataset',
    )
    training_parser.add_argument(
        '--students_c1',
        type=float,
        default=1.,
        help='The bound expansion for the batch data students coefficient',
    )
    training_parser.add_argument(
        '--students_c2',
        type=float,
        default=1.,
        help='The bound expansion for the safety data students coefficient',
    )
    training_parser.add_argument(
    ...
    training_parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help='To use RLOO as the reinforce baseline or not. If RLOO is True, but num_samples_per_prompt = 1, we use the Running average of returns as the Reinforce baseline',
    )
...
def main() -> None:
    ...
    trainer = ReinforceCILagTrainer(args, ds_train_config, ds_eval_config)
    # For Train
    # trainer.train()
    # trainer.save()
