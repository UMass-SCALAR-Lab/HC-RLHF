...
from safe_rlhf.trainers import SupervisedTrainerEMA
from safe_rlhf.utils import gather_log_probabilities, get_all_reduce_mean, is_main_process


class SimPO_PoP_Fixed_EMA_Trainer(SupervisedTrainerEMA):
    TRAINING_TYPE = 'simpo-pop-fixed-ema'
    ...
    def init_models(self) -> None:
        ...
        #     trust_remote_code=self.args.trust_remote_code,
        # )
        self.model_ema, _ = load_pretrained_models(
            self.args.model_name_or_path,
            model_max_length=self.args.max_length,
            padding_side='left',
            auto_model_type=AutoModelForCausalLM,
    ...
    def init_engines(self) -> None:
        ...
        # )
        self.model_ema, *_ = deepspeed.initialize(
    ...
    def loss(
        ...
        **kwargs
        ...
        sequence_log_probs = self.compute_log_probs(  # size = (2 * B, L - 1)
            self.model.module,
            input_ids=torch.cat([pref_chosen_input_ids, pref_rejected_input_ids], dim=0),
            attention_mask=torch.cat([pref_chosen_attention_mask, pref_rejected_attention_mask], dim=0),
        )
        (
        ...
        with torch.no_grad():
            sequence_log_probs_unpref = self.compute_log_probs(  # size = (2 * B, L - 1)
                self.model_ema.module,
                input_ids=torch.cat([unpref_chosen_input_ids, unpref_rejected_input_ids], dim=0),
                attention_mask=torch.cat([unpref_chosen_attention_mask, unpref_rejected_attention_mask], dim=0),
            )
            (
                unpref_chosen_log_probs,  # size = (B, L - 1)
                unpref_rejected_log_probs,  # size = (B, L - 1)
            ) = sequence_log_probs_unpref.chunk(chunks=2, dim=0)
        ...
        for i in range(batch_size):
            ...
            len_pref_chosen_seq_slice = pref_chosen_end_index+1-pref_diverge_index
            len_pref_rejected_seq_slice = pref_rejected_end_index+1-pref_diverge_index
            len_unpref_chosen_seq_slice = unpref_chosen_end_index+1-unpref_diverge_index
            len_unpref_rejected_seq_slice = unpref_rejected_end_index+1-unpref_diverge_index
            ...
            loss_ = -F.logsigmoid(self.scale_coeff * (((pref_chosen_log_prob_/len_pref_chosen_seq_slice)-(pref_rejected_log_prob_/len_pref_rejected_seq_slice))-torch.clamp(((unpref_chosen_log_prob_/len_unpref_chosen_seq_slice)-(unpref_rejected_log_prob_/len_unpref_rejected_seq_slice)),min=self.args.margin_min,max=self.args.margin_max)))
            losses.append(loss_)
            # self.constraint_hist.extend(pop_gap_func.detach().flatten().tolist())
            pref_chosen_sample_rewards.append(self.scale_coeff * (pref_chosen_log_prob_/len_pref_chosen_seq_slice).detach())
            pref_rejected_sample_rewards.append(self.scale_coeff * (pref_rejected_log_prob_/len_pref_rejected_seq_slice).detach())
            unpref_chosen_sample_rewards.append(self.scale_coeff * (unpref_chosen_log_prob_/len_unpref_chosen_seq_slice).detach())
            unpref_rejected_sample_rewards.append(self.scale_coeff * (unpref_rejected_log_prob_/len_unpref_rejected_seq_slice).detach())
    ...
    def train_step(
        ...
        **score_batch_kwargs,
        ...
        with torch.no_grad():
            ...
            # unpref_margins_all = [torch.zeros_like(unpref_reward_margin) for _ in range(dist.get_world_size())]
            # # Gather costs from all devices
            # dist.all_gather(unpref_margins_all, unpref_reward_margin)
            # # Concatenate the gathered tensors
            # unpref_margins_all_hist = torch.cat(unpref_margins_all, dim=0).to(self.args.device)

            # # Prepare an empty tensor to hold gathered values
            # pref_margins_all = [torch.zeros_like(pref_reward_margin) for _ in range(dist.get_world_size())]
            # # Gather costs from all devices
            # dist.all_gather(pref_margins_all, pref_reward_margin)
            # # Concatenate the gathered tensors
            # pref_margins_all_hist = torch.cat(pref_margins_all, dim=0).to(self.args.device)
        ...
        return {
            ...
            # 'misc/pref_margins': pref_margins_all_hist,
            # 'misc/unpref_margins': unpref_margins_all_hist,
        }

    @torch.no_grad()
    def sync_model_to_ema(self):  # tau < 1.0 for EMA, tau = 1.0 for full copy
        # import ipdb; ipdb.set_trace()

        source = self.model.module
        target = self.model_ema.module

        for (name, param), (_, ema_param) in zip(source.named_parameters(), target.named_parameters()):
            # if param.ds_id != ema_param.ds_id:
            #     raise ValueError("Model and EMA model must be partitioned the same way.")

            # with GatheredParameters([param, ema_param], modifier_rank=None):
            #     if param.data.shape != ema_param.data.shape:
            #         raise ValueError(f"Shape mismatch: {param.data.shape} vs {ema_param.data.shape}")
            #     ema_param.data.mul_(self.args.tau).add_(param.data, alpha=1 - self.args.tau)

            # # Step 1: Gather param
            # with deepspeed.zero.GatheredParameters(param, modifier_rank=None):
            #     src_data = param.data.clone()

            # # Step 2: Gather ema_param and update it
            # with deepspeed.zero.GatheredParameters(ema_param, modifier_rank=None):
            #     if ema_param.data.shape != src_data.shape:
            #         raise ValueError(f"Shape mismatch: {src_data.shape} vs {ema_param.data.shape}")
            #     ema_param.data.mul_(self.args.tau).add_(src_data, alpha=1 - self.args.tau)
            ...
            if hasattr(ema_param, 'ds_id') and ema_param.ds_status == ZeroParamStatus.NOT_AVAILABLE:
                with GatheredParameters(ema_param, modifier_rank=None):
                    if ema_param.data.shape != src_data.shape:
                        raise ValueError(f"Shape mismatch: {src_data.shape} vs {ema_param.data.shape}")
                    ema_param.data.mul_(self.args.tau).add_(src_data, alpha=1 - self.args.tau)
            else:
